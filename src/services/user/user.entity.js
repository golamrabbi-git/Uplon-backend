 ...
 */
const createAllowed = new Set(['firstName', 'lastName', 'userName','fullName', 'email', 'password', 'role', 'gender', 'workingDays', 'dob', 'workingHours', 'maxProjectLimit', 'skillsets', 'status']);
const allowedQuery = new Set(['firstName', 'lastName', 'username','fullName', 'page', 'limit', 'id', 'paginate', 'role']);
const ownUpdateAllowed = new Set(['firstName', 'lastName','fullName', 'phone', 'avatar', 'passwordChange', 'data']);
...
export const removeSelected = ({ db }) => async (req, res) => {
  try {
    ...
    ids.map(id => {
      const user =  db.remove({ table: User, key: { _id: { $in: id } } });
